...
#include "../headers/Square.h"																																												//Inclusão da biblioteca de quadrados

#define X_SCREEN 450																																														//Definição do tamanho da tela em pixels no eixo x
#define Y_SCREEN 450																																														//Definição do tamanho da tela em pixels no eixo y
...
unsigned char check_kill(square *killer, square *victim){																																					//Implementação da função que verifica se um projétil acertou um jogador 
	...
	for (bullet *index = killer->tripple_gun->shots; index != NULL; index = (bullet*) index->next){																											//Para todos os projéteis da tripple pistol do atirador 
...
void update_bullets(square *player){																																										//Implementação da função que atualiza o posicionamento de projéteis conforme o movimento dos mesmos 
	...
	for (bullet *index = player->gun->shots; index != NULL;){																																				//Para cada projétil presente na lista de projéteis disparados 
		if (!index->trajectory_x) index->x -= BULLET_MOVE;																																					//Se a trajetória for para a esquerda, atualiza a posição para a esquerda 
		else if (index->trajectory_x == 1) index->x += BULLET_MOVE;																																			//Se a trajetória for para a direita, atualiza a posição para a esquerda 
	...
	previous = NULL;
	for (bullet *index = player->tripple_gun->shots; index != NULL;){																																		//Para cada projétil presente na lista de projéteis disparados 
		if (!index->trajectory_x) index->x -= BULLET_MOVE;																																					//Se a trajetória for para a esquerda, atualiza a posição para a esquerda 
		else if (index->trajectory_x == 1) index->x += BULLET_MOVE;																																			//Se a trajetória for para a direita, atualiza a posição para a esquerda 
		
		if (!index->trajectory_y) index->y -= BULLET_MOVE;																																					//Se a trajetória em y for para cima, atualiza a posição y para cima
		else if (index-> trajectory_y == 2) index->y += BULLET_MOVE;																																		//Se a trajetória em y for para baixo, atualiza a posição y para baixo

		if ((index->x <= 0) || (index->x >= X_SCREEN) || (index->y <= 0) || (index->y >= Y_SCREEN)){																										//Verifica se o projétil saiu das bordas da janela 
			if (previous){																																													//Verifica se não é o primeiro elemento da lista de projéteis 
				previous->next = index->next;																																								//Se não for, salva o próximo projétil 
				bullet_destroy(index);																																										//Chama o destrutor para o projétil atual 
				index = (bullet*) previous->next;																																							//Atualiza para o próximo projétil 
				...
				player->tripple_gun->shots = (bullet*) index->next;																																			//Atualiza o projétil no início da lista 
				bullet_destroy(index);																																										//Chama o destrutor para o projétil atual 
				index = player->tripple_gun->shots;																																							//Atualiza para o próximo projétil 
			}
		}
		else{																																																//Se não saiu da tela 
			previous = index;																																												//Atualiza o projétil anterior (para a próxima iteração) 
...
void update_position(square *player_1, square *player_2){																																					//Função de atualização das posições dos quadrados conforme os comandos do controle
	...
	if (player_1->control->trpfire){																																										//Verifica se o primeiro jogador está atirando com a tripple_pistol
		if (!(player_1->tripple_gun->timer)){																																								//Verifica se a tripple_pistol do primeiro jogador não está em cooldown
			square_tripple_shot(player_1);																																									//Se não estiver, faz um disparo 
			player_1->tripple_gun->timer = TRIPPLE_PISTOL_COOLDOWN;																																			// Inicia o cooldown da tripple_pistol
		}
	}
	if (player_2->control->trpfire){																																										//Verifica se o segundo jogador está atirando com a tripple_pistol
		if (!(player_2->tripple_gun->timer)){																																								//Verifica se a tripple_pistol do segundo jogador não está em cooldown
			square_tripple_shot(player_2);																																									//Se não estiver, faz um disparo
			player_2->tripple_gun->timer = TRIPPLE_PISTOL_COOLDOWN;																																			//Inicia o cooldown da tripple_pistol
		}
	}
	...
	while(1){																																																//Laço servidor do jogo
		...
		if (p1k || p2k){																																													//Verifica se algum jogador foi morto 
			...
			if (event.type == ALLEGRO_EVENT_TIMER){																																							//O evento tipo 30 indica um evento de relógio, ou seja, verificação se a tela deve ser atualizada (conceito de FPS)
	    		...
	    		for (bullet *index = player_1->tripple_gun->shots; index != NULL; index = (bullet*) index->next) al_draw_filled_circle(index->x, index->y, 2, al_map_rgb(255, 0, 0));						//Insere as balas existentes disparadas pela tripple pistol do primeiro jogador na tela 
	    		if (player_1->tripple_gun->timer) player_1->tripple_gun->timer--;																															//Atualiza o cooldown da tripple pistol do primeiro jogador 
	    		for (bullet *index = player_2->tripple_gun->shots; index != NULL; index = (bullet*) index->next) al_draw_filled_circle(index->x, index->y, 2, al_map_rgb(0, 0, 255));						//Insere as balas existentes disparadas pela tripple pistol do pelo segundo jogador na tela 
	    		if (player_2->tripple_gun->timer) player_2->tripple_gun->timer--; 																															//Atualiza o cooldown da tripple pistol do segundo jogador 
	    					
				al_flip_display();																																											//Insere as modificações realizadas nos buffers de tela
			}
			else if ((event.type == ALLEGRO_EVENT_KEY_DOWN) || (event.type == ALLEGRO_EVENT_KEY_UP)){																										//Verifica se o evento é de botão do teclado abaixado ou levantado
				if (event.keyboard.keycode == ALLEGRO_KEY_A) joystick_left(player_1->control);																												//Indica o evento correspondente no controle do primeiro jogador (botão de movimentação à esquerda)
				else if (event.keyboard.keycode == ALLEGRO_KEY_D) joystick_right(player_1->control);																										//Indica o evento correspondente no controle do primeiro jogador (botão de movimentação à direita)
				else if (event.keyboard.keycode == ALLEGRO_KEY_W) joystick_up(player_1->control);																											//Indica o evento correspondente no controle do primeiro jogador (botão de movimentação para cima)
				else if (event.keyboard.keycode == ALLEGRO_KEY_S) joystick_down(player_1->control);																											//Indica o evento correspondente no controle do primeiro jogador (botão de movimentação para baixo)
				else if (event.keyboard.keycode == ALLEGRO_KEY_LEFT) joystick_left(player_2->control);																										//Indica o evento correspondente no controle do segundo jogador (botão de movimentação à esquerda)
				else if (event.keyboard.keycode == ALLEGRO_KEY_RIGHT) joystick_right(player_2->control);																									//Indica o evento correspondente no controle do segundo jogador (botão de movimentação à direita)
				else if (event.keyboard.keycode == ALLEGRO_KEY_UP) joystick_up(player_2->control);																											//Indica o evento correspondente no controle do segundo jogador (botão de movimentação para cima)
				else if (event.keyboard.keycode == ALLEGRO_KEY_DOWN) joystick_down(player_2->control);																										//Indica o evento correspondente no controle do segundo jogador (botão de movimentação para baixo)
				else if (event.keyboard.keycode == ALLEGRO_KEY_C) joystick_fire(player_1->control);																											//Indica o evento correspondente no controle do primeiro joagdor (botão de disparo - c) 				
				else if (event.keyboard.keycode == ALLEGRO_KEY_RCTRL) joystick_fire(player_2->control);																										//Indica o evento correspondente no controle do segundo joagdor (botão de disparo - ctrl dir) 
				else if (event.keyboard.keycode == ALLEGRO_KEY_Z) joystick_trpfire(player_1->control);																										//Indica o evento correspondente no controle do primeiro jogador (botão de disparo da tripple pistol - z)
				else if (event.keyboard.keycode == ALLEGRO_KEY_RSHIFT) joystick_trpfire(player_2->control);																									//Indica o evento correspondente no controle do segundo jogador (botão de disparo da tripple pistol - shift dir)
			}																																			
