...
square* square_create(unsigned char side, unsigned char face, unsigned short x, unsigned short y, unsigned short max_x, unsigned short max_y){			//Implementação da função "square_create"
	...
	new_square->tripple_gun = tripple_pistol_create();																									//Insere o elemento de disparo triplo do quadrado
...
void square_tripple_shot(square *element){																												//Implementação da função "square_tripple_shot"
	bullet *shot;

	if (!element->face) shot = tripple_pistol_shot(element->x - element->side/2, element->y, element->face, element->tripple_gun);						//Quadrado atira o triple_shot para a esquerda 
	else if (element->face == 1) shot = tripple_pistol_shot(element->x + element->side/2, element->y, element->face, element->tripple_gun);				//Quadrado atira o triple_shot para a direita 
	if (shot) element->tripple_gun->shots = shot;
}

void square_destroy(square *element){																													//Implementação da função "square_destroy"
	
	tripple_pistol_destroy(element->tripple_gun);																										//Destrói a arma tripla do quadrado
