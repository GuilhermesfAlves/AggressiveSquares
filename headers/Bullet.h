...
typedef struct {																																//Definição da estrutura de um pŕojétil (!)
	...
	unsigned char trajectory_x;																													//Trajetória da bala
	unsigned char trajectory_y;
	struct bullet *next; 																														//Próxima bala, se houver, formando uma lista encadeada (!)
} bullet;																																		//Definição do nome da estrutura (!)

bullet* bullet_create(unsigned short x, unsigned short y, unsigned char trajectory_x, unsigned char trajectory_y, bullet *next);												//Protótipo da função de criação de uma bala (!)
